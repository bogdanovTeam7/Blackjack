...
public class StatisticPaneGeneralController {
	...
	void initialize() {
		...
		Text startDate = new Text(generalStatistic.getStartDate()
				.format(formatter));
		startDate.setFont(Font.font(Constants.getFont(), FontWeight.BOLD, 14));
		gameRoundInfoGridPane.add(startDate, 1, 2);

		Text endDate = new Text(generalStatistic.getEndDate()
				.format(formatter));
		endDate.setFont(Font.font(Constants.getFont(), FontWeight.BOLD, 14));
		...
		countOfCardsInPlay.setFont(Font.font(Constants.getFont(), FontWeight.BOLD, 14));
		gameRoundInfoGridPane.add(countOfCardsInPlay, 1, 4);

		Text dillerInfo = new Text(diller.getName());
		dillerInfo.setFont(Font.font(Constants.getFont(), FontWeight.BOLD, 14));
		...
		for (ListIterator<Player> it = players.listIterator(); it.hasNext();) {
			...
			playerInfo.setFont(Font.font(Constants.getFont(), FontWeight.BOLD, 14));
			if (index % 2 != 0) {
